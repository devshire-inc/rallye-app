...
function AppRoutes() {
  ...
  return (
    <Routes>
      ...
      {/* ---------------------------------------------------------------
          Rotas SOB A CASCA. O `AppShell` é montado por esta ÚNICA rota de
          layout e recebe as telas por `<Outlet/>`: navegar entre duas
          rotas daqui troca só o miolo, e a casca — sidebar, bottom nav,
          topbar — é reconciliada em vez de desmontada.

          É isso que faz a animação do indicador do `BottomNav` existir no
          app: um elemento recém-montado não transiciona, ele nasce na
          posição final. Antes, cada uma destas ~62 rotas renderizava o
          próprio `<AppShell>`, então toda navegação remontava a casca e a
          cápsula saltava. No Storybook a animação sempre funcionou porque
          lá o componente fica montado e só a prop `active` muda — esta
          rota de layout reproduz exatamente essa condição.

          TODAS as rotas com casca têm que ficar sob ESTE mesmo elemento:
          duas rotas de layout irmãs seriam duas posições diferentes na
          árvore, e o React remontaria a casca ao pular de uma para a
          outra — exatamente o que se está corrigindo.
          --------------------------------------------------------------- */}
